...
if __name__ == "__main__":
    ...
    tracker_param_cfgs = [
        dict(zip(tracker_param_grids, v))
    ...
    for cfg in tracker_param_cfgs:
        print(cfg["config"])
        print([cfg[m] for m in metrics])

    # compute and plot best metric config
    for m in metrics:
        best_metric_cfg_idx = np.array([cfg[m] for cfg in tracker_param_cfgs]).argmax()

        print(
            f"BEST {m.upper()} CFG: {tracker_param_cfgs[best_metric_cfg_idx]['config']}"
        )

    # TODO
    best_mota_plus_idf1_cfg_idx = np.array(
        [cfg["mota"] + cfg["idf1"] for cfg in tracker_param_cfgs]
    ).argmax()
    print(
        f"BEST MOTA PLUS IDF1 CFG: {tracker_param_cfgs[best_mota_plus_idf1_cfg_idx]['config']}"
    )
