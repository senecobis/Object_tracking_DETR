...
for split in [
    ...
    for dets in ["DPM", "FRCNN", "SDP", "ALL"]:
        ...
        if dets:
            name = f"{name}-{dets}"
        DATASETS[name] = lambda kwargs, split=split, dets=dets: MOT17Wrapper(
            split, dets, **kwargs
        )

use_mot17 = True
if use_mot17:
    DATASETS["MOT17-ALL"] = lambda kwargs, split="ALL", dets="ALL": MOT17Wrapper(
        split, dets, **kwargs
    )
...
for split in ["TRAIN", "TEST", "ALL", "01", "02", "03", "04", "05", "06", "07", "08"]:
    name = f"MOT20-{split}"
    DATASETS[name] = lambda kwargs, split=split: MOT20Wrapper(split, **kwargs)


for split in ["TRAIN", "TEST", "ALL", "01", "02", "05", "06", "07", "09", "11", "12"]:
    name = f"MOTS20-{split}"
    DATASETS[name] = lambda kwargs, split=split: MOTS20Wrapper(split, **kwargs)
