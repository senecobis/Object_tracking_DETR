...
class RandomSizeCrop:
    ...
    def __call__(self, img: PIL.Image.Image, target: dict):
        if self.max_size is None:
            ...
            w = random.randint(
                min(self.min_size[0], img.width), min(img.width, self.max_size[0])
            )
            h = random.randint(
                min(self.min_size[1], img.height), min(img.height, self.max_size[1])
            )
...
class RandomResizeTargets:
    ...
    def __call__(self, img, target=None):
        ...
        for box in target["boxes"]:
            ...
            rescaled_box_image = F.resize(box_img, (scaled_width, scaled_height))
        ...
        for box in target["boxes"]:
            ...
            erase_value = torch.empty([img_c, w, h], dtype=torch.float32).normal_()

            img = F.erase(img, x1, y1, w, h, erase_value, True)
    ...
    def __call__(self, img, target):
        if random.uniform(0, 1) < self.p:
            ...
            if "boxes" in target:
                ...
                lt = torch.max(
                    erased_box[:, None, :2], target["boxes"][:, :2]
                )  # [N,M,2]
                rb = torch.min(
                    erased_box[:, None, 2:], target["boxes"][:, 2:]
                )  # [N,M,2]
