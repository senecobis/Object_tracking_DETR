...
class Tracker:
    ...
    def __init__(
        ...
        if self.generate_attention_maps:
            assert hasattr(
                self.obj_detector.transformer.decoder.layers[-1], "multihead_attn"
            ...
            hook = multihead_attn.register_forward_hook(add_attention_map_to_data)
            attention_data["hooks"].append(hook)
    ...
    def public_detections_mask(self, new_det_boxes, public_det_boxes):
        ...
        if self.public_detections == "center_distance":
            item_size = [
                ((box[2] - box[0]) * (box[3] - box[1])) for box in new_det_boxes
            ]
    ...
    def step(self, blob):
        ...
        if num_prev_track:
            ...
            track_query_boxes = track_query_boxes / torch.tensor(
                [orig_size[0, 1], orig_size[0, 0], orig_size[0, 1], orig_size[0, 0]],
                dtype=torch.float32,
            )
...
class Track(object):
    ...
    def __init__(self, pos, score, track_id, hs_embed, mask=None, attention_map=None):
        self.id = track_id
        self.pos = pos
