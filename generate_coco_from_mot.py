...
def generate_coco_from_mot(
    ...
    annotations["categories"] = [
        {"supercategory": "pedestrian", "name": "pedestrian", "id": 1}
    ]
    ...
    for seq in seqs:
        ...
        for i, img in enumerate(sorted(seg_list_dir)):
            ...
            os.symlink(
                os.path.join(os.getcwd(), root_split_path, seq, "img1", img),
    ...
    annotation_id = 0
    img_file_name_to_id = {
        img_dict["file_name"]: img_dict["id"] for img_dict in annotations["images"]
    }
    for seq in seqs:
        # GT FILE
        ...
        if mots:
            gt_file_path = os.path.join(
                root_split_mots_path, seq.replace("MOT17", "MOTS20"), "gt", "gt.txt"
            )
        ...
        if mots:
            ...
            with open(gt_file_path, "r") as gt_file:
                ...
                for row in reader:
                    if int(row[6]) == 1 and (
                        seq in MOT_15_SEQS_INFO or int(row[7]) == 1
                    ):
                        bbox = [
                            float(row[2]),
                            float(row[3]),
            ...
            if mots_vis:
                ...
                if os.path.isfile(gt_file_mots):
                    ...
                    for (
                        frame_id,
                        frame_annotations,
                    ) in seq_annotations_per_frame.items():
...
def check_coco_from_mot(
    coco_dir="data/MOT17/mot17_train_coco",
    annotation_file="data/MOT17/annotations/mot17_train_coco.json",
    img_id=None,
):
    """
...
if __name__ == "__main__":
    ...
    if args.mots20:
        ...
        for i in range(4):
            ...
            generate_coco_from_mot(
                f"mots20_train_{i + 1}_coco", seqs_names=train_seqs, mots=True
            )
            generate_coco_from_mot(
                f"mots20_val_{i + 1}_coco", seqs_names=val_seqs, mots=True
            )
        ...
        for i in range(0, 4):
            ...
            generate_coco_from_mot(
                f"mot20_train_{i + 1}_coco", seqs_names=train_seqs, data_root=data_root
            )
            generate_coco_from_mot(
                f"mot20_val_{i + 1}_coco", seqs_names=val_seqs, data_root=data_root
            )
        ...
        for i in range(0, 7):
            ...
            generate_coco_from_mot(f"mot17_train_{i + 1}_coco", seqs_names=train_seqs)
            generate_coco_from_mot(f"mot17_val_{i + 1}_coco", seqs_names=val_seqs)
